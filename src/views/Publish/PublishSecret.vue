...
export default {
  ...
  methods: {
    ...
    drawTextInImage() {
      ...
      const ctx = this.imageSelected.getContext();
    ...
    updateBackgroundImage() {
      ...
      this.background = this.imageSelected.generateDataUrl('image/jpeg', 0.8);
    ...
    changeBgColor() {
      ...
      if (form.backgroundSelected === currentLength) form.backgroundSelected = 0;
      else form.backgroundSelected++;
    },
    showError() {
      this.$toast.open({
        duration: 3000,
    ...
    multilinesCanvas(text, opts) {
        const ctx = this.imageSelected.getContext('2d');
        // Default options
        if (!opts) opts = {};
 ...
 opts.rect = {
        ...
        for (var line of lines)
          // Fill or stroke
          {
 if (opts.stroke) ctx.strokeText(line.text.trim(), line.x, line.y);
          else ctx.fillText(line.text.trim(), line.x, line.y);
}
